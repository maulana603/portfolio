...
# using one by one interaction assumption.
...
eij = {}
i = 0
with open('MJ_original_pot.txt', 'r') as f:
    # reading each line
    for line in f:
        # reading each energy
        j = 0
...
eir = {}
with open('MJ_original_pot_eir.txt', 'r') as f:
    # reading each line
    for line in f:
        # reading each energy
        i = 0
...
# open output file
f = open('MJOO_PCA_-5_-1_result.txt', 'w')
f.write('Analyzed structure = {}\n'.format(doc_id))
f.write("Energy matrix = MJ new potential one by one\n")
f.write("Sequence =\n")
...
for residue in res_list:
    if residue.get_resname() in Jernigan_aa_names:
        ...
        # Extracting CA atoms with SASA parameter treshold from residues
        if residue.has_id("CA") and residue.rSASA > 0.2:
...
# target_atom = chains[0].child_list[0]["CA"]
# list_sequence.append(target_atom)

# Choose lowest pair for close atom
for atom in atoms:
    atom.Epair = 10.0
    res_atom = atom.get_parent().get_resname()
    for i in aaj3:
        if potential[(res_atom, i)] < atom.Epair:
            atom.Epair = potential[(res_atom, i)]
            atom.pair = i
...
for residue in res_start:
    ...
    while len(sequence_pair) < length:
        ...
        # Choose the lowest neigbor atom
        lowest_neighbor = 10.0
        for atom in close_atoms:
            if atom.Epair < lowest_neighbor:
                lowest_atom = atom
                lowest_neighbor = atom.Epair
    ...
    # Assigning value to attribute of each residue
    ...
    for a in sequence_pair:
        if a == "GLY":
            ...
            residue.sequence_pair += three_to_onej(a.pair)
