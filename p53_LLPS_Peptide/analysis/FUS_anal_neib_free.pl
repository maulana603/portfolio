...
my ($peptide_score, $peptide_score_rev, $merged_position, $merged_score_min, $merged_score_min_rev, $merged_score_rev, $merged_score_specific, $merged_score_total);
my @peptide1 = ();
my @peptide2 = ();
my @KK_seq = ();
...
for ($i=0; $i < ($length1-$length2+1); $i++){
   if ($i < (286-$length2+1) || $i <(422-$length2+1) && $i > 371-1 || $i <($length1-$length2+1) && $i > 453-1){
      $peptide_score = 0;
      $peptide_score_rev = 0;
      for ($j=0; $j < $length2; $j++){
         $peptide_score += $KK_r[$peptide1[$j+$i]][$peptide2[$j]];
         $peptide_score_rev += $KK_r[$peptide1[$j+$i]][$peptide2[$length2-$j-1]];
      }
