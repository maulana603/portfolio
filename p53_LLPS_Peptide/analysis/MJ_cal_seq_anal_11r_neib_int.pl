...
for ($i=1; $i < ($length1-$length2+1-1); $i++){
   for ($j=0; $j < $length2; $j++){
      $peptide_score[$i] += ($MJ_r[$peptide1[$j+$i]][$peptide2[$j]]+$MJ_r[$peptide1[$j+$i+1]][$peptide2[$j]]+$MJ_r[$peptide1[$j+$i-1]][$peptide2[$j]]);
      $peptide_score_rev[$i] += ($MJ_r[$peptide1[$j+$i]][$peptide2[$length2-$j-1]]+$MJ_r[$peptide1[$j+$i-1]][$peptide2[$length2-$j-1]]+$MJ_r[$peptide1[$j+$i+1]][$peptide2[$length2-$j-1]]);
