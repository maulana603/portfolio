...
my ($peptide_loc_global, $peptide_score_global, $peptide_seq_global);
my @peptide1 = ();
my @peptide2 = ();
my @peptide_score = ();
...
$peptide_score_global = 1000;
$peptide_seq_global = 0;
...
for ($i=1; $i < ($length1-$length2+1-1); $i++){
   ...
   if ($peptide_score[$i]< $peptide_score_global){
      $peptide_loc_global = $i+1;
      $peptide_score_global = $peptide_score[$i];
      $peptide_seq_global = $peptide_seq_write;
   }
   $peptide_seq_write = "";
}
printf("\n");
printf("%s %s %s", $peptide_loc_global, $peptide_score_global, $peptide_seq_global);
