...
for ($i=0; $i < ($length1-$length2+1); $i++){
    for($l=0; $l < ($peptide_level-1); $l++){
        ...
        for ($j=0; $j < $length2; $j++){
            ...
            for ($k=1; $k < 21; $k++){
                if (($MJ_r[$peptide1[$j+$i]][$peptide_seq[$i][$j][$l]]) <  ($MJ_r[$peptide1[$j+$i]][$k])){
                    if ($score_compare >  ($peptide_score[$i][$l] - $MJ_r[$peptide1[$j+$i]][$peptide_seq[$i][$j][$l]] + $MJ_r[$peptide1[$j+$i]][$k])){
                        $peptide_position = $j; #position in designed sequence
                        $numb_compare = $k; #amino acid
                        $score_compare = ($peptide_score[$i][$l] - $MJ_r[$peptide1[$j+$i]][$peptide_seq[$i][$j][$l]] + $MJ_r[$peptide1[$j+$i]][$k]);
                    }
                }
            #printf("%s %s %s %s %s\n",$i, $j, $k, $peptide1[$j+$i], $MJ_r[$peptide1[$j+$i]][$k]);
            }
...
for ($k=0; $k < ($length1-$length2+1); $k++){
    for($l=0; $l < $peptide_level; $l++){
        $merged_score_total = 0;
        $merged_score_rev = 0;
        $merged_score_min[$k][$l] = 1000;
        my @merged_score = ();
        for ($i=0; $i < ($length3-$length2+1); $i++){
            if ($i < (94-$length2) || $i < (325-$length2) && $i > 294 || $i <($length3-$length2) && $i > 357){
                $peptide_score2 = 0;
                $peptide_score_rev = 0;
                for ($j=0; $j < $length2; $j++){
                    $peptide_score2 += ($MJ_r[$peptide3[$j+$i]][$peptide_seq[$k][$j][$l]]);
                    $peptide_score_rev += ($MJ_r[$peptide3[$j+$i]][$peptide_seq[$k][$length2-$j-1][$l]]);
